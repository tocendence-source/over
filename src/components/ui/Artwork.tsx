...
export function Artwork({ id, interactive = false, className }: { id: ArtworkId; interactive?: boolean; className?: string }) {
  ...
  // Prefer the same-origin original; fall back to the interim CDN copy, then to
  // the in-brand placeholder. Candidates are deduped so a record that sets only
  // one URL never retries the same request.
  const sources = useMemo(() => [...new Set([item.local, item.remote].filter((source): source is string => Boolean(source)))], [item]);
  ...
  ) : <img src={src} alt={item.alt} width="640" height="640" loading="lazy" decoding="async" referrerPolicy="no-referrer" onError={advance} />;
  ...
  return (
    <>
      ...
      <dialog ref={dialog} className="lightbox" aria-label={item.alt} onClose={() => { setOpen(false); trigger.current?.focus({ preventScroll: true }); }} onClick={(event) => { if (event.target === event.currentTarget) dialog.current?.close(); }}>
        {open && <img src={src} alt={item.alt} width="640" height="640" referrerPolicy="no-referrer" onError={() => { dialog.current?.close(); advance(); }} />}
